...
void _KnnSearchCPU(NanoFlannIndexHolderBase *holder,
                   ...
                   const T *const points, size_t num_queries,
                   const T *const queries, const size_t dimension, int knn,
                   bool ignore_query_point, bool return_distances,
  ...
  auto holder_ = static_cast<NanoFlannIndexHolder<METRIC, T, TIndex> *>(holder);
        ...
        for (size_t i = r.begin(); i != r.end(); ++i) {
          ...
          for (size_t valid_i = 0; valid_i < num_valid; ++valid_i) {
            ...
            if (ignore_query_point &&
                points_equal(&queries[i * dimension], &points[idx * dimension],
                             dimension)) {
              continue;
            }
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<size_t> &r) {
        for (size_t i = r.begin(); i != r.end(); ++i) {
          ...
          std::copy(neighbors_indices[i].begin(), neighbors_indices[i].end(),
                    &indices_ptr[start_idx]);

          if (return_distances) {
            std::copy(neighbors_distances[i].begin(),
                      neighbors_distances[i].end(), &distances_ptr[start_idx]);
          }
        }
      });
}
...
void _RadiusSearchCPU(NanoFlannIndexHolderBase *holder,
                      int64_t *query_neighbors_row_splits, size_t num_points,
                      const T *const points, size_t num_queries,
                      const T *const queries, const size_t dimension,
                      const T *const radii, bool ignore_query_point,
                      bool return_distances, bool normalize_distances,
                      bool sort, OUTPUT_ALLOCATOR &output_allocator) {
  if (num_queries == 0 || num_points == 0 || holder == nullptr) {
    std::fill(query_neighbors_row_splits,
              query_neighbors_row_splits + num_queries + 1, 0);
    TIndex *indices_ptr;
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<size_t> &r) {
        ...
        for (size_t i = r.begin(); i != r.end(); ++i) {
          ...
          holder_->index_->radiusSearch(&queries[i * dimension], radius,
          ...
          for (const auto &idx_dist : search_result) {
            if (ignore_query_point &&
                points_equal(&queries[i * dimension],
                             &points[idx_dist.first * dimension], dimension)) {
              continue;
            }
...
template <class T, class TIndex, class OUTPUT_ALLOCATOR, int METRIC>
void _HybridSearchCPU(NanoFlannIndexHolderBase *holder, size_t num_points,
                      const T *const points, size_t num_queries,
                      const T *const queries, const size_t dimension,
                      const T radius, const int max_knn,
                      bool ignore_query_point, bool return_distances,
                      OUTPUT_ALLOCATOR &output_allocator) {
  if (num_queries == 0 || num_points == 0 || holder == nullptr) {
      ...
      [&](const tbb::blocked_range<size_t> &r) {
        ...
        for (size_t i = r.begin(); i != r.end(); ++i) {
          size_t num_results = holder_->index_->radiusSearch(
              &queries[i * dimension], radius_squared, ret_matches, params);
...
BuildKdTree(size_t num_points, const T *const points, size_t dimension,
            const Metric metric) {
...
void KnnSearchCPU(NanoFlannIndexHolderBase *holder,
                  ...
                  const T *const points, size_t num_queries,
                  const T *const queries, const size_t dimension, int knn,
                  const Metric metric, bool ignore_query_point,
                  bool return_distances, OUTPUT_ALLOCATOR &output_allocator) {
...
void RadiusSearchCPU(NanoFlannIndexHolderBase *holder,
                     ...
                     const T *const points, size_t num_queries,
                     const T *const queries, const size_t dimension,
                     const T *const radii, const Metric metric,
                     bool ignore_query_point, bool return_distances,
                     bool normalize_distances, bool sort,
                     OUTPUT_ALLOCATOR &output_allocator) {
#define FN_PARAMETERS                                                          \
  holder, query_neighbors_row_splits, num_points, points, num_queries,         \
...
///
template <class T, class TIndex, class OUTPUT_ALLOCATOR>
void HybridSearchCPU(NanoFlannIndexHolderBase *holder, size_t num_points,
                     const T *const points, size_t num_queries,
                     const T *const queries, const size_t dimension,
                     const T radius, const int max_knn, const Metric metric,
                     bool ignore_query_point, bool return_distances,
                     OUTPUT_ALLOCATOR &output_allocator) {
#define FN_PARAMETERS                                                          \
