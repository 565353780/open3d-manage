...
public:
  ...
  MatrixToVector3d(const Eigen::MatrixXd &mat);

  static bool compare_triangles(const Eigen::Vector3i &t1,
                                const Eigen::Vector3i &t2);

  static Eigen::MatrixXi
  DelaunayKNN(const Eigen::MatrixXd &knn_locations_including_self,
  ...
  static void TotalCurvaturePCD(const Eigen::MatrixXd &V,
                                const Eigen::MatrixXd &N, Eigen::VectorXd &k_S,
                                int K);
};
