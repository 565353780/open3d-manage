...
std::vector<int>
TotalCurvaturePointCloud::Where(int i, const Eigen::MatrixXi &inArray) {
  std::vector<int> res;
  int idx = 0;
...
TotalCurvaturePointCloud::MatrixToVector3d(const Eigen::MatrixXd &mat) {
...
Eigen::MatrixXi TotalCurvaturePointCloud::DelaunayKNN(
  ...
  n_plane = {n(0), n(1), n(2)};
  r0 = {knn_locations_including_self(0, 0), knn_locations_including_self(0, 1),
  ...
  for (size_t i = 0; i < knn_locations_2d.rows(); ++i) {
    points_with_indices.emplace_back(
        K::Point_3(knn_locations_2d(i, 0), knn_locations_2d(i, 1), 0.0), i);
  }
  ...
  for (Delaunay::Finite_faces_iterator face = dt.finite_faces_begin();
    ...
    Eigen::Vector3i triangle(face->vertex(0)->info(), face->vertex(1)->info(),
                             face->vertex(2)->info());
  ...
  adjacent_triangles_idx_local =
      all_triangles(adjacent_triangles_mask, Eigen::indexing::all); // 5, 3
  // Converting indices back to original global indices
  Eigen::MatrixXi adjacent_triangles_idx_x =
      idx(Eigen::indexing::all,
          adjacent_triangles_idx_local(Eigen::indexing::all, 0));
  Eigen::MatrixXi adjacent_triangles_idx_y =
      idx(Eigen::indexing::all,
          adjacent_triangles_idx_local(Eigen::indexing::all, 1));
  Eigen::MatrixXi adjacent_triangles_idx_z =
      idx(Eigen::indexing::all,
          adjacent_triangles_idx_local(Eigen::indexing::all, 2));
...
double TotalCurvaturePointCloud::PerTriangleLaplacianTriangleFanCurvature(
    const Eigen::MatrixXi &adjacent_triangles_idx, const Eigen::MatrixXd &V,
    const Eigen::MatrixXd &N) {
  Eigen::MatrixXd triangle, n_triangle_face, v_triangle_fan, n_triangle_fan, x,
      y, z;
  ...
  for (int i = 0; i < N_triangles; i++) {
    ...
    Eigen::MatrixXd AB_mat =
        triangle(0, Eigen::indexing::all) - triangle(1, Eigen::indexing::all);
    Eigen::MatrixXd AC_mat =
        triangle(0, Eigen::indexing::all) - triangle(2, Eigen::indexing::all);
...
void TotalCurvaturePointCloud::TotalCurvaturePCD(const Eigen::MatrixXd &V,
                                                 const Eigen::MatrixXd &N,
                                                 Eigen::VectorXd &k_S, int K) {
  ...
  for (int i = 0; i < V.rows(); i++) {
    ...
    Eigen::MatrixXd knn_locations_including_self =
        V(idx_vec, Eigen::indexing::all);
    int n_rows = idx_vec.size();
    Eigen::MatrixXi idx = Eigen::MatrixXi::Zero(
        1, n_rows); // Create a column vector of zeros with the same number of
                    // rows as idx_vec
    for (int i = 0; i < n_rows; i++) {
      idx(0, i) = idx_vec[i];
    }
    Eigen::MatrixXi adjacent_triangles_idx =
        TotalCurvaturePointCloud::DelaunayKNN(knn_locations_including_self,
                                              idx);
    k_S(i) = TotalCurvaturePointCloud::PerTriangleLaplacianTriangleFanCurvature(
        adjacent_triangles_idx, V, N);
  }
}
